UPDATE team_years ty1
    LEFT JOIN (
        ...
        FROM (
                 SELECT max(ty2.id) as id, sum(scr.points * scr.confirmed) AS pp, count(DISTINCT(m.id)) as mc
                    FROM team_years ty2
                             ...
                             INNER JOIN scout_ratings scr ON scr.matchevent_log_id=mel.id
